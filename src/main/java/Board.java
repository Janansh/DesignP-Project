...
public class Board extends JPanel implements Runnable, Commons {
	...
	public void gameInit() {
		...
		ImageIcon ii = null; // replaced by ResourceManager where needed

		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 6; j++) {
				Alien alien = factory.createAlien(alienX + 18 * j, alienY + 18 * i);
				alien.setImage(ResourceManager.getInstance().getImage(alienpix));
				aliens.add(alien);
			}
		}

		player = factory.createPlayer();
		shot = new Shot(); // keep default invisible shot for initialization
	...
	public void animationCycle() {
		if (deaths == NUMBER_OF_ALIENS_TO_DESTROY) {
			ingame = false;
			message = "Parabns! Voc salvou a galxia!";
		}
		...
		while (it.hasNext()) {
			...
			if (alien.isVisible()) {
				...
				if (y > GROUND - ALIEN_HEIGHT) {
					havewon = false;
					ingame = false;
					message = "Aliens esto invadindo a galxia!";
				}
